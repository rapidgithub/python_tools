...
def load_config(path: str = "config.props") -> configparser.ConfigParser:
    ...
    if not read_files:
        logging.warning("Config file %s not found; using environment/defaults where possible.", path)
    return cfg
...
def fetch_soup(session: requests.Session, url: str, timeout: int = DEFAULT_TIMEOUT) -> Optional[BeautifulSoup]:
    ...
    logging.error("Unable to connect to %s after %d attempts: %s", url, RETRY_ATTEMPTS, last_exc)
    return None
...
def process_table(table_body) -> Tuple[List[str], List[List[List[str]]]]:
    ...
    Returns a tuple (hrefs, data_lists)
      ...
      - data_lists: list of table row lists for the table (each element is a list of rows; each row is list of cell texts)
    """
    ...
    for row in rows:
        ...
        if cols:
            data.append(cols)
    return hrefs, [data]  # data wrapped in list for consistency with possible multiple tables processed


def extract_values_from_data_lists(data_lists: List[List[List[str]]]) -> List[str]:
    """
    Given a list of data tables (each a list of rows), extract second-column values where possible.
...
def print_details(session: requests.Session, base_url: str, table_soup, table_type: str) -> List[str]:
    """Given a table soup (the table element from the base page), find query links and visit each."""
    ...
    for href in hrefs:
        ...
        if not tables:
            ...
            continue
        hrefs2, data_lists = process_table(tables[0])
...
def main():
    ...
    if len(tables) < 3:
        logging.error("Unexpected page layout: expected at least 3 tables on %s, found %d", base_url, len(tables))
    else:
        open_table = tables[1]
    ...
    print("List of open: {}\nList of closed: {}".format(open_results, closed_results))
